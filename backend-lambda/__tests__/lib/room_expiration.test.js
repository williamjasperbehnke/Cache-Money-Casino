...
describe("room_expiration", () => {
  const originalTimeout = process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS;
  const originalLegacy = process.env.BLACKJACK_MULTI_ROOM_IDLE_SECONDS;

  afterEach(() => {
    if (originalTimeout === undefined) delete process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS;
    else process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS = originalTimeout;
  ...
  it("uses default timeout when env is not set", () => {
    delete process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS;
  ...
  it("uses configured timeout in seconds", () => {
    process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS = "90";
    expect(getRoomIdleTimeoutMs()).toBe(90 * 1000);
  });

  it("treats non-positive timeout as disabled", () => {
    process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS = "0";
  ...
  it("marks room expired when timeout exceeded", () => {
    process.env.BLACKJACK_MULTI_ROOM_IDLE_TIMEOUT_SECONDS = "60";
