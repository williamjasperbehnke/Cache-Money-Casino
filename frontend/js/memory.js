...
export class MemoryGame {
  ...
  updateStats() {
    ...
    if (this.ui.multiplierEl) {
      this.ui.multiplierEl.textContent = state.memory.completed
        ? `${Number(state.memory.multiplier || 0).toFixed(2)}x`
        : "--";
    }
  }
  ...
  updateUI() {
    ...
    if (this.ui.clearBtn) {
      this.ui.clearBtn.disabled = state.memory.inRound && !state.memory.completed;
    }
    this.updateStats();
    this.renderBoard();
  }
